...
export default function Certifications() {
  ...
  return (
    <section id="certifications" className="relative w-full h-screen snap-start overflow-hidden text-white">
      ...
      <div className="absolute inset-0 z-20 flex flex-col items-center justify-start pt-20 overflow-y-auto md:overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          {/* Unified Certifications Grid - Desktop & Tablet */}
          <h3 className="mt-16 text-2xl font-semibold">Certifications &amp; Training</h3>
          <div className="hidden md:grid mt-6 grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 sm:gap-12">
          ...
          {/* Mobile Scroll Snap View */}
          <div className="md:hidden flex-1 overflow-y-auto snap-y snap-mandatory px-2 pt-6 pb-10 space-y-12">
            {certifications.map(cert => (
              <div key={cert.id} className="snap-center flex flex-col items-center">
                <motion.div
                  className="w-full rounded-lg overflow-hidden cursor-pointer border border-transparent hover:border-blue-400 transition duration-300"
                  whileHover={{ scale: 1.05, opacity: 0.9 }}
                  transition={{ type: 'spring', stiffness: 180, damping: 12 }}
                  onClick={() => setOpen(cert)}
                >
