...
export default function Experience() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      entries => entries.forEach(e => e.isIntersecting && setSelected(+e.target.dataset.index)),
      { root: containerRef.current, threshold: 0.6 }
    )
    containerRef.current.querySelectorAll('.exp-slide').forEach(el => obs.observe(el))
    return () => obs.disconnect()
  }, [])

  const scrollTo = i => {
    document.querySelector(`[data-index="${i}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  const cx = size / 2
  const cy = size / 2
  const totalSlices = experiences.length + 1
